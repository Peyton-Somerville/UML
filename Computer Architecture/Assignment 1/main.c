...
int main(int argc, char * argv[])
{
	...
	do {
		...
		printf("mantissa:  0x%6x\t or: ", float_32.f_bits.mantissa);
		for (i = 12; i <= 39; i++) {
			printf("%c", bit_string[i]);
		}

		printf("\nexponent:  0x%2x\t\t or: ", float_32.f_bits.exponent);
		for (i = 2; i <= 10; i++) {
			printf("%c", bit_string[i]);
		}

		printf("\nsign:\t   %x", float_32.f_bits.sign);
		printf("\t\t or: %c\n", bit_string[0]);

		printf("in base10: %g", float_32.floating_value_in_32_bits);
		printf("\t or: %s\n\n", bit_string);

		do {
			printf("\nDo you want to enter another floating point number?\n0: No\n1: Yes\n>> ");
